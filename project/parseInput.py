...
class Node():
    ...
    def _calculateFlowCost(self):
        ...
        EDmodel.add_constraint(EDmodel.sum(
            powerCplexVar[idx] for idx in powerCplexVar) == input.D[self._t])
...
class UC_Network():
    ...
    def createModelFromInput():
        ...
        source = Node("0000000000", -1, isSource=True)
        nodes.append(source)
        ...
        sink = Node("1111111111", input.nPeriodi, isSink=True)
        nodes.append(sink)
...
class UC_Model():
    ...
    def generateUCNetworkModel(myNet):
        ...
        DO_MULTIPROCESSING2 = False
        if DO_MULTIPROCESSING2:
            # Split the nodes in as many chunks as the number of cores and calculate the flow cost in parallel
            manager = multiprocessing.Manager()
            ...
            x = {(arc._n1.id, arc._n2.id): UC_Model.UCNetworkModel.continuous_var(name='x_{0}_{1}'.format(arc._n1.id, arc._n2.id)) for arc in myNet.arcs}
        ...
        print("\nI'm creating the flow conservation constraints")
        ...
        #     c+=1
        #     diffPattern = utils.differenceBetweenID(a._n1.id[0], a._n2.id[0])
        ...
        #     # if (len(diffPattern)>0):
        #     nonValidNodeKeys = utils.f(a._n1.id, a._n2.id,  2, diffPattern, tau)
        ...
        #     # If this arc variable is 1, then all the variables in the notValidCombsAndRangeT must be 0
        #     if len(not_valid_arcs_list)>0:
        #         UC_Model.UCNetworkModel.add(UC_Model.UCNetworkModel.if_then(x[(a._n1.id, a._n2.id)] >= 1, [x[(arc._n1.id, arc._n2.id)] == 0 for arc in not_valid_arcs_list]))
        print("FONE")
        time.sleep(5)
        """
        # get range of time between arc._n2._t+1 and arc._n2.t+min_up-1
        rangeT = range(a._n2.id[1]+1, a._n2.id[1]+min_up)
        # Create tuple with all the combinations of the not valid combinations and the range of time
        notValidCombsAndRangeT = [(x, y) for x in notValidCombs for y in rangeT]
        # If this arc variable is 1, then all the variables in the notValidCombsAndRangeT must be 0
        UC_Model.UCNetworkModel.if_then(x[(a._n1.id, a._n2.id)] >= 1, [x[(i, j)] == 0 for (i, j) in notValidCombsAndRangeT])
        """

        print("\nI added all the constraints")
        # return UC_Model.UCNetworkModel
...
if "__main__" == __name__:
    ...
    arcs = [a1, a2, a3, a4]
    for i in nodes:
        v= [Arc(i,j) for j in nodes if (i,j) in arcs]
        print(len(v))
        print(v[0])
        print(type(v[0]))
    """
